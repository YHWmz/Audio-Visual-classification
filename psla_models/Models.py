...
class EffNetAttention(nn.Module):
    ...
    def forward(self, x, nframes=1056):
        # expect input x = (batch_size, time_frame_num, frequency_bins), e.g., (N, 1000, 128)
        # effnet output y = (batch_size, 1408, time_frame_num//32, frequency_bins//32)
        x = x.unsqueeze(1)
        x = x.transpose(2, 3)
        ...
        x = self.avgpool(x)
        features = x.transpose(2,3)
        out, norm_att = self.attention(features)
        features = features.view(features.shape[0], features.shape[1], features.shape[2]).permute(0, 2, 1)
