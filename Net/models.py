...
class _Resformix(nn.Module):
    def __init__(self, block, layers, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(_Resformix, self).__init__()
    ...
    def forward(self, x, index, mix_up):
        x = self.layer1(x)
        lam = 0
        if(mix_up == 4):
            mix_x, lam = mixup_data(x, index, 1)
            x = torch.cat((x, mix_x)).cuda()
        x = self.layer2(x)
        if(mix_up == 5):
            mix_x, lam = mixup_data(x, index, 2)
            x = torch.cat((x, mix_x)).cuda()
        x = self.layer3(x)
        if(mix_up == 6):
            mix_x, lam = mixup_data(x, index, 1)
...
class ConvBformix(nn.Module):
    def __init__(self, in_channels, out_channels):
        
        super(ConvBformix, self).__init__()
    ...
    def forward(self, input, index, mix_up, mix_index, pool_size=(2, 2), pool_type='avg'):
        ...
        if(mix_up == mix_index[1]):
            mix_x, lam = mixup_data(x, index, 2)
            x = torch.cat((x,mix_x)).cuda()
        
        return x, lam
    
def mixup_data(x, index, alpha=1.0, use_cuda=True):
...
class ResNet38_formix(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num):
        
        super(ResNet38_formix, self).__init__()
        ...
        self.conv_block1 = ConvBformix(in_channels=1, out_channels=64)
        # self.conv_block2 = ConvBlock(in_channels=64, out_channels=64)

        self.resnet = _Resformix(block=_ResnetBasicBlock, layers=[3, 4, 6, 3], zero_init_residual=True)

        self.conv_block_after1 = ConvBformix(in_channels=512, out_channels=2048)
    ...
    def forward(self, input, index, mix_up=0):
        ...
        x, lam = self.conv_block1(x, index, mix_up, [2,3], pool_size=(2, 2), pool_type='avg')
        x = F.dropout(x, p=0.2, training=self.training, inplace=True)

        x, lam = self.resnet(x, index, mix_up) #456
        
        x = F.avg_pool2d(x, kernel_size=(2, 2))
        x = F.dropout(x, p=0.2, training=self.training, inplace=True)
        x, lam = self.conv_block_after1(x, index, mix_up, [7,8], pool_size=(1, 1), pool_type='avg')#78
...
class Wavegram_Logmel_Cnn14(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        import numpy as np
        # Wavegram
        input1 = torch.FloatTensor(librosa.resample(np.array(input.cpu()),orig_sr=4400, target_sr=32000)).cuda()
        a1 = F.relu_(self.pre_bn0(self.pre_conv0(input1[:, None, :])))
        a1 = self.pre_block1(a1, pool_size=4)
        a1 = self.pre_block2(a1, pool_size=4)
        a1 = self.pre_block3(a1, pool_size=4)
        a1 = a1.reshape((a1.shape[0], -1, 32, a1.shape[-1])).transpose(2, 3)
...
class Cnn10(nn.Module):
    def __init__(self,sample_rate, window_size, hop_size, mel, fmin, fmax, classes_num, layer_fc):
        super(Cnn10, self).__init__()
    ...
    def forward(self, input):
        ...
        # x = x.unsqueeze(1)  # (batch_size, 1, time_steps, mel_bins=64)
        #
        # x = x.transpose(1, 3)
        # x = self.bn0(x)
        # x = x.transpose(1, 3)
